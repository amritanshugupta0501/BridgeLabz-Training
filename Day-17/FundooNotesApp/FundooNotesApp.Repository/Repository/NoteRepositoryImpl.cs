...
namespace FundooNotesApp.Models
{
    public class NoteRepositoryImpl : INoteRepository
    {
        ...
        {
            return _context.Note.Include(n => n.Labels).Where(n => n.UserId == userId && n.Trash == false).ToList();
        }
        public Notes GetNoteById(int noteId, int userId)
        {
            return _context.Note.Include(n=>n.Labels).FirstOrDefault(n => n.NoteId == noteId && n.UserId == userId);
        }
        ...
        public bool AddLabelToNote(int noteId, int labelId, int userId)
        {
            var note = _context.Note.Include(n => n.Labels)
                .FirstOrDefault(n => n.NoteId == noteId && n.UserId == userId);
            var label = _context.Label.FirstOrDefault(l => l.LabelId == labelId && l.UserId == userId);
        ...
        public bool DeleteLabelFromNote(int noteId, int labelId, int userId)
        {
            var note = _context.Note.Include(n => n.Labels)
                .FirstOrDefault(n => n.NoteId == noteId && n.UserId == userId);
            var label = _context.Label.FirstOrDefault(l => l.LabelId == labelId && l.UserId == userId);
