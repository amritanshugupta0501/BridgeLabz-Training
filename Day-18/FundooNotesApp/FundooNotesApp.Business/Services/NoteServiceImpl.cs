...
namespace FundooNotesApp.Business
{
    public class NoteServiceImpl : INoteService
    {
        ...
        {
            bool isAdded = _repository.AddLabelToNote(noteId, labelId, userId);
            if (!isAdded)
            {
                throw new Exception("Note or Label not found");
            }
        ...
        {
            bool isDeleted = _repository.DeleteLabelFromNote(noteId, labelId, userId);
            if (!isDeleted)
            {
                throw new Exception("Note or Label not found");
            }
